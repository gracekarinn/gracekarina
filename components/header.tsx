...
export default function Header() {
  ...
  <header className="z-[999] relative">
    ...
    sm:w-[40rem] sm:rounded-full"
      ...
      animate={{ y: 0, x: "-50%", opacity: 1, transition: { type: 'spring', damping: 10, stiffness: 100 }}}
    ></motion.div>
    <nav className='flex fixed top-[0.15rem] left-1/2 h-12 -translate-x-1/2 py-2 
    sm:top-[1.7rem] sm:h-[initial] sm:py-0'>
      <ul className='flex w-[20frem] flex-wrap items-center justify-center
      gap-y-4 text-s font-medium text-gray-500 sm:w-[initial]
      sm:flex-nowrap sm:gap-5'>
        {
          links.map(link => (
            ...
            animate={{ y: 0, opacity: 1, transition: { type: 'spring', damping: 10, stiffness: 100 }}}>
              <Link 
                className={clsx("flex w-full items-center justify-center px-6 py-3 hover:text-gray-950 transition whitespace-nowrap", {"text-gray-950": activeSection === link.name,})}
                 href={link.hash}
                 onClick = {() => {
